...
**One store speaks both dialects rather than two implementations.** They differ in five places — placeholder spelling, `BLOB`/`BYTEA`, `REAL`/`DOUBLE PRECISION`, `INTEGER`/`BIGINT` (SQLite's integer is 64-bit and Postgres's is not), and where the schema version is kept — and every query, migration and test is shared. The alternative is two things that must agree, kept apart, and discovered to disagree by a user. The same suite runs against both backends in CI, which is how a claim that let two Postgres executors take the same run was found before it reached anyone.

That sentence was not true when it was first written. Four test helpers opened SQLite directly instead of going through the shared one, so node samples, reclamations, recoveries and staleness never ran against Postgres — and v0.6.0 shipped with the reclamation ledger unable to record a single drain there. The lesson is not "write more tests": it is that a suite claiming to cover two backends needs one door into it, and a test asserting that no query reaches the driver unrewritten has to know every way to reach the driver. Both are now enforced rather than intended.
